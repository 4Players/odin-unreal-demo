...
void LoadBalancingListener::customEventAction(int playerNr, nByte eventCode, const Object& eventContentObj)
{
	...
	else if(eventCode == 2)
	{
		...
		{
			float x = 0; float y = 0; float z = 0; float rotX = 0; float rotY = 0; float rotZ = 0;
			...
			else if (obj->getType() == TypeCode::OBJECT)
			{
				...
				{
					x = ((ValueObject<float>*)(data + 0))->getDataCopy();
					y = ((ValueObject<float>*)(data + 1))->getDataCopy();
				}
				else
				{
			...
			//#ifdef __UNREAL__
			//			mpView->changePlayerPos(playerNr, (float)(mGridSize-1-y), (float)x, 0.0f); // convert from demo to engine coordinate system on receive (necessary so that builds of this demo for different game engines are compatible to each other)
			//#else
			mpView->changePlayerPos(playerNr, (float)x, (float)y, (float)z, (float)rotX, (float)rotY, (float)rotZ);
			//#endif
		}
		else
...
void LoadBalancingListener::moveLocalPlayer(void)
{
	...
	if(y >= mGridSize)
		y = mGridSize - 2;
	setLocalPlayerPos((float)x, (float)y, 0.0f);*/
}

bool LoadBalancingListener::setLocalPlayerPos(float x, float y, float z, float rotX, float rotY, float rotZ)
{
	...
	{*/
		...
		mLocalPlayer.rotX = rotX;
		mLocalPlayer.rotY = rotY;
		mLocalPlayer.rotZ = rotZ;
...
//#else
		float coords[] ={static_cast<float>(mLocalPlayer.x), static_cast<float>(mLocalPlayer.y), static_cast<float>(mLocalPlayer.z), static_cast<float>(mLocalPlayer.rotX) , static_cast<float>(mLocalPlayer.rotY) , static_cast<float>(mLocalPlayer.rotZ) };
//#endif
		data.put((int)1, coords, 6);


		mpLbc->opRaiseEvent(false, data, 2, RaiseEventOptions().setInterestGroup(0));
		mpView->changePlayerPos(mLocalPlayerNr, mLocalPlayer.x, mLocalPlayer.y, mLocalPlayer.z, mLocalPlayer.rotX, mLocalPlayer.rotY, mLocalPlayer.rotZ);
		return true;
	/*}
	else
		return false;*/
...
bool LoadBalancingListener::setLocalPlayerRot(float x, float y, float z)
{
	...
	//		nByte coords[] ={static_cast<nByte>(mLocalPlayer.y), static_cast<nByte>(mGridSize-1-mLocalPlayer.x)}; // convert from engine to demo coordinate system on send (necessary so that builds of this demo for different game engines are compatible to each other)
	//#else
	float coords[] = { static_cast<float>(mLocalPlayer.rotX), static_cast<float>(mLocalPlayer.rotY), static_cast<float>(mLocalPlayer.rotZ) };
	//#endif
	data.put((int)1, coords, 3);


	mpLbc->opRaiseEvent(false, data, 3, RaiseEventOptions().setInterestGroup(0));
	mpView->changePlayerRot(mLocalPlayerNr, mLocalPlayer.rotX, mLocalPlayer.rotY, mLocalPlayer.rotZ);
	return true;
	/*}
	else
		return false;*/
